...
export async function addMultipleDynamics(req: Request, res: Response) {
   try {
      const metricDynamicsData: MetricDynamics[] = req.body
      for (const metricDynamicsElem of metricDynamicsData) {
         const metricDynamics = new MetricDynamicsSchema(metricDynamicsElem)
         await metricDynamics.save()
      }
      res.status(201).json(metricDynamicsData)
   } catch (error) {
      res.status(500).json({ error: 'Error adding metric' })
   }
}
